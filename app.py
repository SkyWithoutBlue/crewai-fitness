                    ...
                    if mode == "📝 Полный контент-пак (Пост + Сценарий + Воронка в Директ + А/Б Хуки + Telegram)":
                        ...
                        reels_task = Task(
                            description=(
                                ...
                                f"- 🎙️ **ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (высокая плотность разговорной информации)**. Людмила подает глубокую экспертную и научно обоснованную информацию. Текста должно быть много (около 120-150 слов плотного, динамичного текста без пауз), раскрывающего физиологические тонкости темы. Никаких общих фраз и сокращений!\n"
                                f"- **🎬 Режиссура монтажа в поддержку разговора**: Пропиши, как динамичный монтаж (всплывающие тезисы, зумы, акцентные B-roll кадры, графические схемы на экране) помогает зрителю понять эту сложную информацию.\n"
                            ...
                            expected_output="Детализированный сценарий Reels с таймингами, ракурсами, звуками, плотным монтажным планом и обильным дословным дикторским текстом для чтения на камеру.",
                            agent=reels_creator,
                        ...
                        reels_task = Task(
                            description=(
                                ...
                                f"- 🎙️ **ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (максимум полезной разговорной информации)**: Пропиши глубокий, насыщенный фактами и психологическими деталями дословный текст речи Людмилы. Текст должен быть плотным и объемным, чтобы зритель получил максимум пользы в разговорном формате.\n"
                                f"- **🎬 Описание монтажа**: Укажи, как склейки, текстовые плашки и визуальные эффекты помогают понять и удержать эту плотную струю информации."
                            ),
                            expected_output="Сценарий Reels с ракурсами, музыкой, монтажным планом и обильным разговорным дикторским текстом.",
                            agent=reels_creator,
                            output_file=reels_file
                        )
                        ...
                        reels_task = Task(
                            description=(
                                f"Создай сценарий промо-Reels (до 45 сек) для приглашения подписчиков на этот прямой эфир.\n"
                                ...
                                f"- 🎙️ **ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (разговорная экспертность)**: Напиши насыщенный, весомый разговорный текст для Людмилы, подробно раскрывающий ценность тем эфира. Текста должно быть много, он должен звучать авторитетно и плотно.\n"
                                f"- **🎬 Видеомонтаж**: Пропиши динамичный монтаж (подчеркивание фраз крупным шрифтом на экране, динамичные зумы камеры на ключевых словах), помогающий зрителю воспринять эту информацию."
                            ),
                            expected_output="Промо-сценарий Reels с ракурсами, музыкой, монтажными подсказками и весомым дикторским текстом.",
                            agent=reels_creator,
                            output_file=reels_file
                        )
                        ...
                        reels_task = Task(
                            description=(
                                ...
                                "Обязательно напиши **ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (что именно говорить на камеру)** для каждого видео, чтобы Людмила выдавала плотный поток разговорной информации. Сделай этот текст развернутым, емким и глубоким.\n"
                                ...
                                "- **🎙️ Подробный дикторский текст (обильная разговорная информация слово-в-слово):** [Развернутый и объемный текст для озвучки]\n"
                                "- **🎬 Визуальный ряд, Ракурсы и Монтаж (как видео помогает понять информацию):**\n"
                                "  - **0-3 сек:** [Кадр, ракурс, действия, эффекты монтажа]\n"
                                "  - **3-10 сек:** [Кадр, ракурс, действия, эффекты монтажа]\n"
                                "  - **10-15 сек:** [Кадр, ракурс, действия, эффекты монтажа]\n"
                                "- **🎵 Звук / Музыка:** [Описание звука и музыки]\n\n"
                                "Добавь разделительную горизонтальную черту '---' между идеями."
                            ),
                            expected_output="10 детально прописанных идей Reels с четкой структурой, визуалом, схемами монтажа и обильным дикторским текстом для каждой идеи.",
                            agent=reels_creator,
                        ...
                        reels_task = Task(
                            description=(
                                ...
                                "- 🎙️ **ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (разговорная нутрициология)**: Пропиши развернутый дикторский текст, где Людмила подробно объясняет пользу ингредиентов в разговорной манере, давая максимум информации, а не просто перечисляя шаги рецепта.\n"
                                "- **🎬 Эффекты монтажа**: Опиши, как динамичный кулинарный монтаж помогает визуализировать сложные факты о пользе блюда.\n"
                                "- Звуковое сопровождение (ритмичный трек, ASMR-звуки нарезки, шипения сковороды, бурления воды)."
                            ),
                            expected_output="Кулинарный сценарий Reels с ASMR, раскадровкой, монтажным планом и обильным закадровым текстом по нутрициологии.",
                            agent=reels_creator,
                            output_file=reels_file
                        )
                        ...
                        reels_task = Task(
                            description=(
                                ...
                                f"Для каждого из 5 хуков обязательно пропиши **🎙️ ОБИЛЬНЫЙ, ПОЛНЫЙ, слово-в-слово дикторский текст (что говорить на камеру)**. Текст должен быть развернутым, насыщенным разговорными и научными аргументами, создавая прочную основу для качественного видеомонтажа, помогающего подписчику легко усвоить эту информацию."
                            ),
                            expected_output="5 сценариев видеоряда под А/Б хуки с точными ракурсами, монтажными ориентирами и обильным дикторским текстом для озвучки.",
                            agent=reels_creator,
                            output_file=reels_file
                        )
