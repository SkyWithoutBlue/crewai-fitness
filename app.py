...
{BRAND_STYLE.get('description', '')}""",
                        ...
                        writing_task = Task(
                            description=f"""ВАЖНО: Напиши пост СТРОГО на тему '{topic}'. НЕ меняй тему, НЕ выбирай другую тему из исследования. Используй идеи из исследования только как вдохновение для подачи, но тема поста должна быть ИМЕННО: '{topic}'.

ПРАВИЛА ОФОРМЛЕНИЯ ПОСТА ВК (ОБЯЗАТЕЛЬНО СОБЛЮДАЙ ВСЕ):

📐 СТРУКТУРА:
- Хук (первые 2 строки) — вопрос-провокация или неожиданный факт, цепляет сразу
- Вступление — 2-3 предложения, обозначь проблему
- Основная часть — пункты с нумерацией 1️⃣ 2️⃣ 3️⃣ 4️⃣ 5️⃣
- Личная часть — «Я сама через это прошла» (1-2 предложения от лица Людмилы)
- CTA — тёплый призыв как подруге + ссылка chipizubova.online
- Вопрос для комментариев
- Хештеги: 3-5 в конце

✍️ СТИЛЬ ТЕКСТА:
- Обращение на ТЫ (не «вы», не «друзья»)
- Тон: как подруга-эксперт, тёплый но уверенный
- Короткие абзацы — максимум 2-3 предложения
- Между разделами ставь разделитель ⸻
- Простые слова, без англицизмов и сложных терминов
- Если используешь термин — сразу объясни простыми словами

😊 ЭМОДЗИ:
- Используй эмодзи для визуальных списков (🥗🍗🍚 для еды, 💤 для сна и т.д.)
- Каждый пронумерованный пункт начинай с 1️⃣ 2️⃣ 3️⃣
- В конце поста: 🤍 и 💬
- НЕ перегружай — 1-2 эмодзи на абзац максимум

📏 ДЛИНА:
- 2000-4000 символов
- Пост должен хорошо читаться с телефона
- Никаких стен текста

❌ НЕ ДЕЛАЙ:
- НЕ пиши как учебник или лекция
- НЕ используй «друзья», «дорогие», «уважаемые»
- НЕ начинай с «Устали от...» или «Знаете ли вы...»
- НЕ пиши «Приглашаю вас на сайт» — пиши «Заходи: chipizubova.online»
